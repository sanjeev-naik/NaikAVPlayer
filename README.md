...
- **Picture Adjustment & GPU-Accelerated White Balance:** Full-pipeline picture controls (brightness, contrast, saturation, color temperature 3000K–12000K, and green-magenta tint) applicable to all media sources (SDR and HDR). White balance gains are derived from the Planckian locus (Kim et al. cubic approximation) normalized against D65 and perceptual luminance.
  - **GPU Hardware Acceleration for Temperature & Tint:** Pure white-balance adjustments are offloaded directly to the GPU texture pipeline via SDL3 linear color modulation (`SDL_SetTextureColorModFloat`). Frames remain on the zero-copy hardware decoding path (`useNative = true`) in planar YUV (12 bpp) with **~0% CPU overhead** — ensuring fluid 60+ FPS playback on resource-constrained platforms like Raspberry Pi.
  - **Seamless Software Fallback:** Adjusting non-linear/affine parameters (brightness, contrast, saturation) seamlessly invokes the multi-threaded CPU conversion pipeline (`ColorAdjuster`), dynamically capped to the display window dimensions to prevent wasteful processing. On HDR files with tone mapping active, all adjustments fold directly into the tone mapper's lookup tables at zero extra cost.
  - **Live Controls & Persistence:** Accessible via `Shift+C` or the `[Color]` button in the controls dock; settings persist across sessions in `player_settings.txt`.
...
| **`C`** | Toggle HDR -> SDR Tone Mapping Panel (on/off toggle, target & source peak luminance) |
